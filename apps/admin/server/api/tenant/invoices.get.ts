...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (contractIds.length === 0) {
      return successResponse({ invoices: [], total: 0, page, limit }, 'ดึงข้อมูลใบแจ้งหนี้สำเร็จ')
    }
    ...
    const paymentRows = await db
      .select()
      .from(schema.payment)
      .where(inArray(schema.payment.invoiceId, invIds))
    const paymentsByInv: Record<
      string,
      {
        id: string
        amount: string
    ...
    return successResponse({ invoices, total, page, limit }, 'ดึงข้อมูลใบแจ้งหนี้สำเร็จ')
  } catch (error) {
