...
export default defineEventHandler(async (event) => {
  try {
    ...
    const [availableRoomsRow] = await db
      ...
      .from(schema.room)
      .where(
        and(eq(schema.room.propertyId, propertyId), eq(schema.room.status, RoomStatus.AVAILABLE)),
      )
    ...
    if (latestBillingRun) {
      ...
      const byInvStatus = Object.fromEntries(
        billingStats.map((r: { status: string | null; count: number }) => [
          r.status,
          Number(r.count),
        ]),
      )
      billing = {
        ...
        unpaid:
          (byInvStatus[InvoiceStatus.UNPAID] ?? 0) + (byInvStatus[InvoiceStatus.OVERDUE] ?? 0),
      }
    }
    ...
    for (const c of noticeGiven) {
      const termDate =
        c.terminatedDate instanceof Date ? c.terminatedDate : new Date(c.terminatedDate)
