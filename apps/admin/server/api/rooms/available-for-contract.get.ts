...
export default defineEventHandler(async (event) => {
  try {
    ...
    const roomTypes = await db.select().from(schema.roomType).where(inArray(schema.roomType.id, roomTypeIds))
    ...
    const primaryLinks = contractIds.length > 0
      ? await db
          .select()
    ...
    const availableNow: { id: string, roomType: { name: string, basePrice: string, deposit: string }, roomNumber: string, label: string }[] = []
    const availableSoon: { id: string, roomType: { name: string, basePrice: string, deposit: string }, roomNumber: string, label: string, availableDate: Date }[] = []
