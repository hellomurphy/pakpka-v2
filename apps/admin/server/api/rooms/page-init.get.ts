...
function formatRoomForPageInit(
  room: {
    id: string
    roomNumber: string
    status: string | null
  ...
  const activeContract = room.contractId
    ? {
        id: room.contractId,
        tenants: room.tenantId
          ? [
              {
                tenant: { id: room.tenantId, name: room.tenantName, status: room.tenantStatus },
                isPrimary: true,
              },
            ]
          : [],
  ...
  const activeReservation = room.reservationId
    ? {
        ...
        tenant: {
          ...
          name: room.reservationTenantId
            ? (reservationTenantNames[room.reservationTenantId] ?? '')
            : '',
        },
      }
    : null
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (floors.length > 0) {
      ...
      for (const c of activeContracts) {
        ...
        const services = csList.map((cs) => {
          ...
          return {
            ...
            service: s
              ? { name: s.name, defaultPrice: s.defaultPrice }
              : { name: '', defaultPrice: '0' },
          }
        })
      ...
      const reservationByRoom: Record<
        string,
