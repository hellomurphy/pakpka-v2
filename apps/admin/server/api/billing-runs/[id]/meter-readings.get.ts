...
export default defineEventHandler(async (event) => {
  try {
    ...
    const needMeterContractIds = contracts
      ...
      .map(c => c.id)
    const invoicesNeedingReading = invoices.filter(inv => needMeterContractIds.includes(inv.contractId))
    ...
    const previousReadings = prevInvIds.length > 0
      ? await db
          .select()
    ...
    const currentReadings = needReadingInvIds.length > 0
      ? await db
          .select()
          .from(schema.meterReading)
          .where(inArray(schema.meterReading.invoiceId, needReadingInvIds))
      : []
    const currentReadingsMap = new Map<string, { newElec: number | null, newWater: number | null }>()
