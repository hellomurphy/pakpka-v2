...
export const useBillingStore = defineStore("billing", {
  state: () => ({
    billingRuns: [] as any[], // เก็บข้อมูลสรุปของทุกรอบบิล
  ...
  actions: {
    ...
    async fetchBillingRuns(propertyId: string) {
      ...
      try {
        const response = await useApiFetch(
          `/api/billing-runs?propertyId=${propertyId}`,
          {
    ...
    async fetchMeterReadingList(runId: string) {
      ...
      try {
        const response = await useApiFetch(
          `/api/billing-runs/${runId}/meter-readings`,
          {
            showNotification: false,
          }
        );
    ...
    async fetchInvoicesForReview(runId: string, params: any) {
      this.isLoading = true;
    ...
    async saveMeterReadings(runId: string, readings: any[]) {
      const response = await useApiFetch(
        `/api/billing-runs/${runId}/meter-readings`,
        {
          method: "POST",
