...
export const useContractsStore = defineStore('contracts', {
  ...
  actions: {
    ...
    async createContractForExistingTenant(data: Record<string, unknown>) {
    ...
    async giveNoticeOfMoveOut(contractId: string, noticeDate: Date) {
      this.isLoading = true
      try {
        const response = await useApiFetch(`/api/contracts/${contractId}/notice`, {
          method: 'POST',
          body: { noticeDate },
    ...
    async finalizeMoveOut(contractId: string, data: FinalizeMoveOutData) {
      this.isLoading = true
      try {
        const response = await useApiFetch(`/api/contracts/${contractId}/finalize`, {
          method: 'POST',
          body: data,
