...
const { pendingPayments, isLoading, totalPending, itemsPerPage } = storeToRefs(paymentsStore)
...
watch(pendingPayments, (newList) => {
  if (!selectedPayment.value || !newList.some((p) => p.id === selectedPayment.value.id)) {
...
const handlePaymentAction = async ({ paymentId, action }) => {
  const actedPaymentIndex = pendingPayments.value.findIndex((p) => p.id === paymentId)
...
<template>
  <div class="h-full flex flex-col bg-slate-50">
    ...
    >
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">ตรวจสอบการชำระเงิน</h1>
      ...
      <div class="mt-4 md:mt-0">
        <div v-if="isDemoMode">
          <BaseButton variant="secondary" @click="handleGenerateDemo">
