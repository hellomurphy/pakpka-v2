  ...
  >
    <div class="flex-1 overflow-y-auto">
      <div v-if="isLoading" class="p-2 md:p-4 space-y-2">
        <div v-for="n in 5" :key="n" class="h-16 rounded-lg bg-gray-200 animate-pulse" />
      </div>
      <div v-else-if="payments.length === 0" class="p-4 md:p-6 text-center text-sm text-gray-500">
        ...
        >
          <div class="flex justify-between items-baseline gap-2">
            <p class="font-semibold text-sm md:text-base text-gray-800 truncate">
              {{ payment.tenantName }}
              <span class="font-normal text-gray-500">- {{ payment.roomNumber }}</span>
            </p>
    ...
    >
      <BaseButton :disabled="currentPage <= 1" size="sm" variant="secondary" @click="prevPage">
        <ChevronLeftIcon class="h-4 w-4" />
      </BaseButton>
      <p class="text-xs text-gray-500">หน้า {{ currentPage }} / {{ totalPages }}</p>
      <BaseButton
