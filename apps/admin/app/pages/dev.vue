  ...
  <div class="p-8 max-w-2xl mx-auto">
    ...
    <div class="mt-8 bg-white shadow rounded-lg">
      ...
      <div class="px-6 py-4 space-y-4">
        <!-- Alert -->
        <div class="flex items-start bg-red-50 border-l-4 border-red-400 p-4 rounded">
          <ExclamationTriangleIcon class="w-5 h-5 text-red-500 flex-shrink-0 mr-3" />
          <div class="text-sm text-red-700">
            การกระทำนี้จะลบข้อมูลทั้งหมดในฐานข้อมูลปัจจุบัน และสร้างข้อมูลทดสอบชุดใหม่ขึ้นมาแทนที่
          </div>
        </div>
...
<script setup>
import { ref } from 'vue'
import { ServerStackIcon, ExclamationTriangleIcon } from '@heroicons/vue/20/solid'
...
const seedDatabase = async () => {
  ...
  try {
    ...
    isError.value = true
    resultMessage.value = error.data?.statusMessage || 'An unknown error occurred.'
  } finally {
    loading.value = false
  }
}
