...
onMounted(async () => {
  ...
  if (action === 'renew' && fromContractId) {
    ...
    if (response.success) {
      ...
      const primaryTenant = oldContract.tenants.find((t) => t.isPrimary)?.tenant
...
const openAddTenantModal = () => addModal.value?.open()
  ...
  <div class="p-6 md:px-8 bg-white rounded-xl shadow-md h-full flex flex-col">
    <header class="mb-4">
      <div class="flex flex-col sm:flex-row sm:items-baseline sm:gap-x-4">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">จัดการผู้เช่า</h1>
    ...
    >
      <div class="overflow-x-auto">
        <div class="inline-flex items-center bg-slate-200/60 rounded-full p-1 space-x-1">
          <button
            v-for="tab in tabs"
            :key="tab.id"
            :class="[
          ...
          >
            ...
            <span
              v-if="tab.count > 0"
              :class="[
                activeTab === tab.id ? 'bg-blue-100 text-blue-600' : 'bg-white text-gray-600',
      ...
      <div class="flex-shrink-0 flex items-center gap-x-3">
        <div class="w-full sm:w-52">
          <BaseSearchInput v-model="searchQuery" placeholder="ค้นหาผู้เช่า..." debounce="500" />
        </div>
        <BaseButton :icon="PlusIcon" @click="openAddTenantModal">
    ...
    <EditTenantModal ref="editModal" :tenant="tenantToEdit" @success="refreshTenantList" />

    <NoticeMoveOutModal ref="noticeMoveOutModal" @success="onActionSuccess" />
    <ViewContractModal ref="viewContractModal" />
    <FinalizeMoveOutModal ref="finalizeMoveOutModal" @success="onActionSuccess" />
