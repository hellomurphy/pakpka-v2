...
<template>
  <BaseModal v-if="tenantInfo" v-model="isModalOpen" max-width="lg" :persistent="true">
    <template #title> จัดการการย้ายออก: {{ tenantInfo.name }} </template>

    <div class="mt-4">
      <form id="finalize-form" class="space-y-4" @submit.prevent="onSubmit">
        <div class="p-4 bg-slate-50 rounded-lg text-sm">
          <p><span class="font-medium">ห้อง:</span> {{ tenantInfo.roomNumber }}</p>
          <p>
    ...
    <template #footer>
      <div class="w-full flex justify-end gap-x-3">
        ...
        <BaseButton type="submit" form="finalize-form" :loading="isLoading" variant="primary">
          ยืนยันการย้ายออก
        </BaseButton>
      </div>
    </template>
