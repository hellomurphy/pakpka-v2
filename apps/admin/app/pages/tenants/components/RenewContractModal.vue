...
const open = (tenantData) => {
  ...
  resetForm({
    values: {
      endDate: dayjs(contract.value.endDate).add(1, 'year').format('YYYY-MM-DD'),
      hasNewTerms: false,
...
<template>
  <BaseModal v-model="isModalOpen" max-width="lg">
    ...
    <div v-if="tenant && contract" class="mt-4 space-y-6">
      ...
      <form id="renew-contract-form" class="space-y-4" @submit="onFormSubmit">
        <BaseDatePicker
          v-model="endDate"
          v-bind="endDateAttrs"
        ...
        <div v-if="values.hasNewTerms" class="p-4 bg-slate-50 rounded-lg border border-slate-200">
          <p class="text-sm text-slate-600">
            เมื่อกด "ดำเนินการต่อ" ระบบจะนำคุณไปยังหน้าสร้างสัญญาใหม่ โดยมีข้อมูลเดิมเป็นค่าเริ่มต้น
            เพื่อให้คุณกำหนดเงื่อนไขใหม่ได้
          </p>
        </div>
      </form>
    </div>
    ...
    <template #footer>
      <div class="flex-1 flex justify-end gap-x-3">
        ...
        <BaseButton type="submit" form="renew-contract-form" :loading="isLoading">
