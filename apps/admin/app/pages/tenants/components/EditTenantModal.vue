...
<template>
  <BaseModal v-if="tenant" v-model="isModalOpen" maxWidth="lg" :persistent="true">
    <template #title> แก้ไขข้อมูล: {{ tenant.name }} </template>

    <div class="mt-4">
      <form id="edit-tenant-form" @submit.prevent="onSubmit" class="space-y-4">
        <BaseInput
          v-model="name"
          v-bind="nameAttrs"
          label="ชื่อ-นามสกุล"
    ...
    <template #footer>
      <div class="w-full flex justify-end gap-x-3">
        ...
        <BaseButton type="submit" form="edit-tenant-form">
