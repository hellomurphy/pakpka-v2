...
const { currency, shortDate, fullDateTime } = useFormatters();
...
const {
  ...
  pending,
  error,
} = await useFetch<any>(`/api/rooms/${roomId}`);
...
const formatDateTime = (date: string | Date) => {
  if (!date) return "-";
  return dayjs(date).locale("th").format("D MMM BB HH:mm");
};
...
// --- Computed ---
const calculateUsage = (invoice: any, type: 'electricity' | 'water') => {
  const currentValue = invoice.meterReadings?.[type]?.value;
  if (!currentValue) return '-';

  // หาค่าเดือนก่อนหน้า
  const currentIndex = room.value?.invoices?.findIndex((inv: any) => inv.id === invoice.id);
  ...
  <div v-if="pending" class="bg-white max-w-7xl mx-auto p-4 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex-1">
        ...
        <div class="h-8 w-32 bg-gray-300 rounded animate-pulse mb-2"></div>
    ...
    <div class="border-b border-gray-200">
      <div class="flex space-x-8">
        <div v-for="n in 4" :key="n" class="h-10 w-24 bg-gray-200 rounded animate-pulse"></div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        <div class="space-y-6">
          <div v-for="n in 3" :key="n">
            <div class="h-6 w-40 bg-gray-300 rounded animate-pulse mb-4"></div>
            <div class="space-y-3">
  ...
  <div v-else-if="room" class="bg-white rounded-xl  mx-auto p-4 space-y-6">
    ...
    <div class="bg-white shadow rounded-lg p-6 lg:p-8 min-h-[400px]">
      <div v-show="activeTab === 'overview'">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
                  ...
                  <div v-if="room.lastMeterReading?.electricity?.date" class="text-xs text-yellow-600 mt-1">
                ...
                <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                  ...
                  <div v-if="room.lastMeterReading?.water?.date" class="text-xs text-blue-600 mt-1">
                  ...
                  <div class="text-sm space-y-1">
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium text-yellow-900">
                        {{ room.currentContract?.electricityBillingType === 'PER_UNIT' ? 'คิดต่อหน่วย' : 'อัตราคงที่' }}
                      </span>
                    </div>
                    <div class="flex justify-between">
                  ...
                  <div class="text-sm space-y-1">
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium text-blue-900">
                        {{ room.currentContract?.waterBillingType === 'PER_UNIT' ? 'คิดต่อหน่วย' : 'อัตราคงที่' }}
                      </span>
                    </div>
                    <div class="flex justify-between">
        ...
        <!-- ห้องจอง -->
        <div v-else-if="room.status === 'RESERVED' && room.activeReservation" class="text-center py-12">
          ...
          <div class="mt-6 max-w-md mx-auto bg-blue-50 p-4 rounded-lg">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-blue-900">{{ room.activeReservation.tenant.name }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-blue-700">เบอร์:</span>
                <span class="font-medium text-blue-900">{{ room.activeReservation.tenant.phone }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-blue-700">วันเริ่ม:</span>
                <span class="font-medium text-blue-900">{{ formatDate(room.activeReservation.startDate) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else class="text-center py-12">
          <div class="text-gray-300 text-5xl mb-4">🔧</div>
          <h3 class="text-lg font-medium text-gray-900">{{ getStatusLabel(room.status) }}</h3>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="inv in room.invoices" :key="inv.id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-4 whitespace-nowrap text-sm text-right font-mono text-yellow-700">
                  {{ inv.meterReadings?.electricity?.value || '-' }}
                </td>
                <td class="px-4 py-4 whitespace-nowrap text-sm text-right font-mono text-blue-700">
                ...
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                  <template v-if="inv.meterReadings?.electricity?.date || inv.meterReadings?.water?.date">
                    {{ formatDate(inv.meterReadings?.electricity?.date || inv.meterReadings?.water?.date) }}
                  </template>
                  <template v-else>-</template>
                </td>
              </tr>
      ...
      <div v-show="activeTab === 'history'">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="tenant in room.pastTenants" :key="tenant.tenantId" class="hover:bg-gray-50">
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                  {{ tenant.name }}
                </td>
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
