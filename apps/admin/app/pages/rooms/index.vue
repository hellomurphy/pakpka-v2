...
const { floors, roomTypes, rooms, isLoading, initialFloorId } =
...
const filteredRooms = computed(() => {
  ...
  return sortedRooms.filter((room) => {
    const floorMatch =
      !selectedFloorId.value || room.floorId === selectedFloorId.value;
    const statusMatch =
      selectedStatus.value.value === "ALL" ||
      room.status === selectedStatus.value.value;
    const roomTypeMatch =
      selectedRoomTypeId.value === "ALL" ||
      room.roomTypeId === selectedRoomTypeId.value;
...
const trickFetch = async () => {
  if (selectedFloorId.value) {
    await roomsStore.fetchRoomsForFloor(
      propertyId.value,
...
const deleteRoom = async (room) => {
  const confirmed = await showConfirm({
    ...
    message:
      "การลบห้องจะทำได้ก็ต่อเมื่อไม่มีสัญญาผูกอยู่ การกระทำนี้ไม่สามารถย้อนกลับได้",
    intent: "danger",
  });
  if (confirmed) {
...
const openAddTenantModal = (room) =>
  addTenantModal.value?.open({ roomId: room.id });
const openViewContractModal = (room) => viewContractModal.value?.open(room);
const initiateMoveOut = (room) =>
  noticeMoveOutModal.value?.open(room.activeContract);
const openCreateReservationModal = (room) =>
  createReservationModal.value?.open(room);
...
<template>
  <div class="flex flex-col h-full bg-white rounded-xl shadow-sm">
      ...
      >
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div class="flex-1 grid grid-cols-2 sm:flex sm:flex-row gap-3">
            <Listbox v-model="selectedRoomTypeId" as="div" class="relative flex-1 sm:w-auto sm:min-w-[12rem]">
              <ListboxButton
              ...
              >
                <span class="block truncate">{{
                  roomTypeOptions.find((rt) => rt.value === selectedRoomTypeId)
                    ?.label || "ประเภททั้งหมด"
                }}</span>
                <span
            ...
            <Listbox v-model="selectedStatus" as="div" class="relative flex-1 sm:w-auto sm:min-w-[12rem]">
              <ListboxButton
                class="relative w-full cursor-default rounded-lg bg-white py-2.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-600 sm:text-sm"
              >
    ...
    <main class="flex-1 overflow-y-auto p-6 md:px-8 bg-slate-50/50">
      ...
      >
        <div v-for="n in 10" :key="n" class="h-40 rounded-xl bg-gray-200"></div>
      </div>
      <div
      ...
      <div v-else class="text-center py-20">
        ...
        <p class="mt-1 text-sm text-gray-500">
          ไม่พบห้องที่ตรงกับเงื่อนไขการกรองของคุณ
        </p>
        <div class="mt-6">
          <BaseButton
            @click="
