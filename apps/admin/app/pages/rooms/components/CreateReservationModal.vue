...
const reservationSchema = toTypedSchema(
  z.object({
    ...
    endDate: z.coerce
      .date()
      .min(new Date(), { message: "วันที่หมดอายุต้องไม่ใช่วันในอดีต" }),
  })
);
...
<template>
  <BaseModal v-if="roomToReserve" v-model="isModalOpen" maxWidth="md">
    <template #title>
      สร้างการจองสำหรับห้อง: {{ roomToReserve.roomNumber }}
    </template>

    <div class="mt-4">
      <p class="text-sm text-gray-600 mb-4">
        การจองจะเปลี่ยนสถานะห้องเป็น "จองแล้ว" ชั่วคราว
        เพื่อป้องกันไม่ให้คนอื่นทำสัญญา
      </p>
      <form id="reservation-form" @submit.prevent="onSubmit" class="space-y-4">
        <BaseSelect
          v-model="tenantId"
    ...
    <template #footer>
      <div class="w-full flex justify-end gap-x-3">
        ...
        <BaseButton
          ...
          :loading="reservationsStore.isLoading"
        >
          ยืนยันการจอง
        </BaseButton>
      </div>
