...
const filters = [
  ...
  {
    id: 'ending_soon',
    name: `ใกล้หมดอายุ (${activeProperty.value?.contractEndingSoonDays || 60} วัน)`,
  },
  ...
  <div class="p-6 md:px-8 bg-white rounded-xl shadow-md h-full flex flex-col">
    <header class="pb-4 border-b border-gray-200">
      <div class="flex flex-col sm:flex-row sm:items-baseline sm:gap-x-4">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">จัดการสัญญา</h1>
        <p class="mt-1 sm:mt-0 text-base text-gray-500">ศูนย์รวมข้อมูลสัญญาทั้งหมดในระบบ</p>
      </div>
    </header>

    <div class="py-4 flex flex-col gap-4">
      <div class="flex-1">
        <BaseSearchInput v-model="searchQuery" placeholder="ค้นหาตามเลขห้อง หรือ ชื่อผู้เช่า..." />
      </div>
    ...
    <div class="flex-1 overflow-y-auto -mx-6 md:-mx-8 p-5">
      ...
      >
        <template #cell-room="{ item }">
          <span class="font-semibold text-gray-800">{{ item.room.roomNumber }}</span>
        </template>
        ...
        <template #cell-actions="{ item }">
          <BaseButton variant="secondary" size="sm" @click="goToContractDetail(item)">
            ดู/แก้ไข
          </BaseButton>
        </template>
      </BaseTable>
