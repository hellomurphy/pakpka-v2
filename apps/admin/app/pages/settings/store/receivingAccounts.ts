...
export const useReceivingAccountsStore = defineStore("receivingAccounts", {
  ...
  getters: {
    bankAccounts: (state) =>
      state.accounts.filter((a) => a.type === "BANK_ACCOUNT"),
    promptPayAccounts: (state) =>
      state.accounts.filter((a) => a.type === "PROMPTPAY"),
  },
  ...
  actions: {
    ...
    async fetchAccounts(propertyId: string) {
      ...
      try {
        const response = await useApiFetch(
          `/api/receiving-accounts?propertyId=${propertyId}`,
    ...
    async updateAccount(accountId: string, data: AccountPayload) {
      const response = await useApiFetch(
        `/api/receiving-accounts/${accountId}`,
        {
          method: "PUT",
          body: data,
        }
