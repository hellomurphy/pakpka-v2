...
const isStructureChanged = computed(() => {
  return settings.value ? floorCountInput.value !== settings.value.totalFloors : false
})
...
const generateRooms = (floorIndex) => {
  ...
  if (isNaN(start) || isNaN(end) || start > end || !generator.selectedTypeId) return
  ...
  floor.rooms.sort((a, b) => a.roomNumber.localeCompare(b.roomNumber, undefined, { numeric: true }))
  generator.startRoom = ''
  generator.endRoom = ''
}
...
const handleUpdateStructure = async () => {
  ...
  await settingsStore.updateFloorStructure(propertyId.value, floorCountInput.value)
}
</script>
  ...
  <div v-if="settings" class="space-y-8">
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-base font-semibold leading-7 text-gray-900">โครงสร้างอาคาร</h3>
        ...
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
          <div class="flex items-end gap-x-3">
            <BaseInput
              ...
              class="flex-1"
            />
            <BaseButton :disabled="!isStructureChanged" @click="handleUpdateStructure">
              อัปเดต
            </BaseButton>
          </div>
          <BaseSelect
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-base font-semibold leading-7 text-gray-900">การตั้งค่าการดำเนินงาน</h3>
        <p class="mt-1 text-sm text-gray-500">กำหนดกฎเกณฑ์มาตรฐานสำหรับการบริหารจัดการห้องพัก</p>
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-6 border-t border-gray-200 pt-4">
          <BaseInput
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center">
          <div>
            <h3 class="text-base font-semibold leading-7 text-gray-900">ประเภทห้องพัก (แม่แบบ)</h3>
            <p class="mt-1 text-sm text-gray-500">
              สร้างประเภทห้องเพื่อใช้เป็นแม่แบบในการเพิ่มห้องพัก
            </p>
          </div>
          <BaseButton :icon="PlusIcon" @click="openAddRoomTypeModal"> เพิ่มประเภทห้อง </BaseButton>
        </div>
        <div class="mt-4 border-t border-gray-200">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50/50">
              <tr>
                <th class="py-3 pl-6 text-left text-xs font-semibold uppercase text-gray-500">
                  ชื่อประเภท
                </th>
                <th class="py-3 px-3 text-left text-xs font-semibold uppercase text-gray-500">
                  ค่าเช่าพื้นฐาน
                </th>
                <th class="py-3 px-3 text-left text-xs font-semibold uppercase text-gray-500">
                  ค่ามัดจำ
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              <tr v-for="rt in settings.roomTypes" :key="rt.id" class="hover:bg-gray-50">
                ...
                <td class="relative py-3 pr-6 text-right">
                  <Menu as="div" class="relative inline-block text-left">
                    <MenuButton class="p-2 text-gray-400 hover:text-gray-600 rounded-full">
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-base font-semibold leading-7 text-gray-900">จัดการห้องพักรายชั้น</h3>
        <p class="mt-1 text-sm text-gray-500">
          ใช้เครื่องมือสร้างห้องเป็นชุด จากนั้นตรวจสอบและกด "บันทึกการเปลี่ยนแปลง" ที่หน้าหลัก
        </p>
      </div>
      <div class="border-t border-gray-200">
        <div v-if="settings.floors && settings.floors.length > 0" class="divide-y divide-gray-200">
          <div v-for="(floor, index) in settings.floors" :key="floor.floorNumber">
            <button
            ...
            >
              <div class="mt-4 p-4 bg-white rounded-lg ring-1 ring-gray-200">
                <p class="text-sm font-medium text-gray-700">เครื่องมือสร้างห้องเป็นชุด</p>
                ...
                <BaseButton class="mt-3" :icon="PlusIcon" @click="generateRooms(index)">
                  สร้างห้อง (ตัวอย่าง)
                </BaseButton>
              </div>
              <div class="mt-4">
                ...
                <p v-else class="mt-2 text-sm text-gray-500">ยังไม่มีห้องในชั้นนี้</p>
              </div>
            </div>
          </div>
        </div>
    ...
    />
    <EditRoomModal ref="editRoomModal" :room="roomToEdit" :room-types="settings.roomTypes" />
  </div>
</template>
