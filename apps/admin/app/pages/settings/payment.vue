...
const { bankAccounts, promptPayAccounts, isLoading } = storeToRefs(accountsStore)
  ...
  <div class="space-y-6">
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-base font-semibold leading-7 text-gray-900">ช่องทางการรับชำระเงิน</h3>
      ...
      <div v-show="activeSubTab === 'bank_account'">
        <div class="px-4 py-5 sm:p-6">
          <div class="flex justify-between items-center mb-4">
            <p class="text-sm text-gray-600">จัดการรายชื่อบัญชีธนาคารสำหรับรับโอนเงิน</p>
            <BaseButton :icon="PlusIcon" size="sm" @click="openAddModal('BANK_ACCOUNT')">
              เพิ่มบัญชี
            </BaseButton>
          </div>
          <div v-if="isLoading" class="text-center py-10 text-gray-500">กำลังโหลด...</div>
          <ul v-else-if="bankAccounts.length > 0" role="list" class="divide-y divide-gray-100">
            <li
            ...
            >
              ...
              <div class="flex shrink-0 items-center gap-x-4">
                <BaseSwitch
                  ...
                  @update:model-value="handleToggleActive(account)"
                />
      ...
      <div v-show="activeSubTab === 'promptpay'">
        <div class="px-4 py-5 sm:p-6">
          <div class="flex justify-between items-center mb-4">
            <p class="text-sm text-gray-600">จัดการข้อมูลพร้อมเพย์สำหรับสร้าง Dynamic QR Code</p>
            <BaseButton
          ...
          <div v-if="isLoading" class="text-center py-10 text-gray-500">กำลังโหลด...</div>
          <ul v-else-if="promptPayAccounts.length > 0" role="list" class="divide-y divide-gray-100">
            <li
          ...
          <div v-else class="text-center py-10 border-2 border-dashed border-gray-200 rounded-lg">
            ...
            <BaseButton :icon="PlusIcon" size="sm" class="mt-4" @click="openAddModal('PROMPTPAY')">
    ...
    <AddEditPaymentMethodModal ref="addEditModal" @success="fetchAccountsData" />
  </div>
</template>
