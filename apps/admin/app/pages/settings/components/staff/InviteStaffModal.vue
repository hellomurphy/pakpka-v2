...
const [nameForReference, nameForReferenceAttrs] = defineField('nameForReference')
...
<template>
  <BaseModal v-model="isModalOpen" max-width="lg">
    ...
    <form v-if="!invitationLink" id="invite-staff-form" class="mt-6 space-y-4" @submit="onSubmit">
      <BaseInput
    ...
    <div v-else class="mt-4">
      <h3 class="text-base font-semibold text-green-700">สร้างลิงก์คำเชิญสำเร็จ!</h3>
      ...
      <div class="mt-4 flex items-center gap-2 rounded-md bg-slate-100 p-3">
        ...
        <BaseButton size="sm" @click="copyLink"> คัดลอก </BaseButton>
      </div>
    </div>

    <template #footer>
      <div v-if="invitationLink" class="w-full flex justify-between items-center">
        <BaseButton variant="secondary" @click="inviteAnother"> เชิญคนถัดไป </BaseButton>
