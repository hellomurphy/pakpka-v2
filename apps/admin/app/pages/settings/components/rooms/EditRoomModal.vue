...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    console.log(`Updating Room ${props.room.id} with new typeId: ${values.typeId}`)
...
<template>
  <BaseModal v-if="room" v-model="isModalOpen" max-width="md" :persistent="true">
    <template #title> แก้ไขห้อง: {{ room.number }} </template>

    <form id="edit-room-form" class="mt-4" @submit.prevent="onSubmit">
      <BaseSelect
        v-model="typeId"
        v-bind="typeIdAttrs"
    ...
    <template #footer>
      <BaseButton variant="secondary" class="ml-3" @click="closeModal"> ยกเลิก </BaseButton>
      <BaseButton type="submit" form="edit-room-form" :loading="isLoading" class="ml-3">
