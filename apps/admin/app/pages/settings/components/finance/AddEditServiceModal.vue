...
const onSubmit = handleSubmit(async (values) => {
  ...
  if (isEditMode.value) {
    response = await servicesStore.updateService(
      service.value.id,
      values,
      propertyId
    );
...
<template>
  <BaseModal v-model="isModalOpen" maxWidth="lg" :persistent="true">
    <template #title>
      {{
        isEditMode
          ? `แก้ไขบริการ: ${service && service.name}`
          : "เพิ่มบริการใหม่"
      }}
    </template>

    <form id="service-form" @submit.prevent="onSubmit" class="mt-6 space-y-5">
      <BaseInput
        v-model="name"
        v-bind="nameAttrs"
        label="ชื่อบริการ"
