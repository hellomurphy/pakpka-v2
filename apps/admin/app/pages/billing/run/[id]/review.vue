...
const getUtilityCost = (items) => {
  if (!items) return 0
  return items
    .filter((item) => item.description.includes('ค่าไฟ') || item.description.includes('ค่าน้ำ'))
...
<template>
  <div class="p-6 md:px-8 bg-white rounded-xl shadow-md min-h-full flex flex-col">
    <header class="mb-4">
      <NuxtLink
      ...
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">ตรวจสอบใบแจ้งหนี้</h1>
    </header>
      ...
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex-1 min-w-0">
          <p v-if="activeRun" class="text-lg font-semibold text-gray-800">
            ...
            <span class="text-blue-600">{{ readyToSendCount }} ฉบับพร้อมส่ง</span>
          </p>
        </div>
    ...
    <div class="py-4 flex flex-col sm:flex-row gap-4">
      <div class="flex-1">
        <BaseSearchInput v-model="searchQuery" placeholder="ค้นหาตามชื่อผู้เช่า หรือ เลขห้อง..." />
      </div>
      <div class="w-full sm:w-56">
        <BaseSelect v-model="sortBy" :options="sortOptions" label-class="hidden" />
      </div>
    </div>
    ...
    <div class="flex-1 -mx-6 md:-mx-8">
      ...
      >
        ...
        <template #cell-tenantInfo="{ item }">
          ...
          <div class="text-sm text-gray-500">ห้อง {{ item.contract.room.roomNumber }}</div>
        </template>
        ...
        <template #cell-utilityCost="{ item }">
          <span class="text-gray-600">{{ currency(getUtilityCost(item.items)) }}</span>
        </template>

        <template #cell-totalAmount="{ item }">
          <span
            class="font-semibold"
            :class="[item.status === 'PAID' ? 'text-green-600' : 'text-blue-600']"
        ...
        <template #cell-actions="{ item }">
          <div class="flex items-center justify-end gap-x-2">
            <BaseButton variant="secondary" size="sm" @click="openInvoiceDetailModal(item)">
              <component
