...
const isMeterReadingStepActive = computed(() => activeRun.value?.status === 'PENDING_METER_READING')
...
const isMeterReadingFullyComplete = computed(() => {
  ...
  return activeRun.value.meterReadingCompletedCount >= activeRun.value.meterReadingRequired
})
...
const isProcessFinished = computed(
  () => activeRun.value?.status === 'SENDING' || activeRun.value?.status === 'COMPLETED',
)
</script>

<template>
  <div v-if="isLoading" class="p-6 text-center text-gray-500">กำลังโหลดข้อมูลรอบบิล...</div>
    ...
    <div class="space-y-6 max-w-3xl">
      <div class="p-5 rounded-xl bg-white ring-1 ring-gray-900/5">
        <div class="flex items-start gap-x-4">
          ...
          <div>
            <h2 class="font-semibold text-gray-900">ขั้นตอนที่ 1: สร้างใบแจ้งหนี้ฉบับร่าง</h2>
      ...
      <div
        ...
        :class="[isMeterReadingStepActive ? 'ring-blue-500 shadow-lg' : 'ring-gray-900/5']"
      >
        <div class="flex items-start gap-x-4">
          <div
            class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full"
          ...
          <div>
            <h2 class="font-semibold text-gray-900">ขั้นตอนที่ 2: จดบันทึกมิเตอร์</h2>
            ...
            <div class="mt-4">
              <NuxtLink :to="`/billing/run/${runId}/meter-reading`">
                <BaseButton :disabled="!isStep1Complete">
                  {{ isMeterReadingFullyComplete ? 'ดู/แก้ไขข้อมูลมิเตอร์' : 'ไปที่หน้าจดมิเตอร์' }}
                </BaseButton>
              </NuxtLink>
            </div>
          </div>
      ...
      >
        <div class="flex items-start gap-x-4">
          ...
          <div>
            <h2 class="font-semibold text-gray-900">ขั้นตอนที่ 3: ตรวจสอบและส่งใบแจ้งหนี้</h2>
