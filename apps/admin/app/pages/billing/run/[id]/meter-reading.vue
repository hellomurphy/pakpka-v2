...
// --- Computed Properties ---
const completedCount = computed(() => list.value.filter((item) => item.isCompleted).length)
...
const handleUpdateAndFlow = (updatedRoom, flowTo) => {
  const index = list.value.findIndex((r) => r.invoiceId === updatedRoom.invoiceId)
  if (index !== -1) {
    list.value[index] = {
      ...list.value[index],
      ...updatedRoom,
  ...
  <div class="h-full bg-slate-100">
    <template v-if="!roomInFocus">
      <div class="flex flex-col h-full">
        <header class="flex-shrink-0 bg-white p-4 border-b shadow-sm sticky top-0 z-10">
          <div class="flex justify-between items-center max-w-5xl mx-auto">
            <NuxtLink :to="`/billing/run/${runId}`">
              <BaseButton variant="secondary" size="sm" :icon="ArrowLeftIcon">กลับ</BaseButton>
            </NuxtLink>
        ...
        <main class="flex-1 overflow-y-auto p-4">
          <div v-if="isLoading" class="text-center py-10 text-slate-500">กำลังโหลด...</div>
    ...
    <MeterReadingFocusView
      ...
      :room="roomInFocus"
      :next-room-exists="
        list.findIndex((r) => r.invoiceId === roomInFocus.invoiceId) < list.length - 1
      "
