...
const elecWarning = computed(() => {
  if (!debouncedReadings.value.elec || debouncedReadings.value.elec === '' || elecHardBlock.value)
    return null
...
const waterWarning = computed(() => {
  if (
    !debouncedReadings.value.water ||
    debouncedReadings.value.water === '' ||
    waterHardBlock.value
  )
    return null
...
watch(
  ...
  (currentReadings, oldReadings) => {
    ...
    if (currentReadings.elec !== oldReadings.elec || currentReadings.water !== oldReadings.water) {
      debouncedSave()
    }
  },
  { deep: true },
  ...
  <div class="fixed inset-0 bg-slate-100 z-50 flex flex-col">
    <header class="flex-shrink-0 bg-white p-4">
      <div class="flex justify-between items-center max-w-md mx-auto">
        <h2 class="text-xl font-bold text-slate-900">ห้อง {{ room.roomNumber }}</h2>
        <button class="p-2 text-slate-500 hover:text-slate-800" @click="$emit('close')">
          <XMarkIcon class="w-6 h-6" />
        </button>
      </div>
    </header>

    <main class="flex-1 flex flex-col justify-between p-4 max-w-md mx-auto w-full">
      <!-- ✨ Hard Block Errors (ด่านที่ 1) -->
      <div v-if="elecHardBlock || waterHardBlock" class="space-y-3 mb-4">
        <!-- Electricity Hard Block -->
        <div v-if="elecHardBlock" class="p-4 rounded-lg bg-red-100 border-2 border-red-500">
          <div class="flex items-start gap-2">
            <ExclamationTriangleIcon class="w-5 h-5 flex-shrink-0 mt-0.5 text-red-600" />
            <div class="flex-1">
              <p class="text-sm font-bold text-red-900">⚡ {{ elecHardBlock.message }}</p>
        ...
        <!-- Water Hard Block -->
        <div v-if="waterHardBlock" class="p-4 rounded-lg bg-red-100 border-2 border-red-500">
          <div class="flex items-start gap-2">
            <ExclamationTriangleIcon class="w-5 h-5 flex-shrink-0 mt-0.5 text-red-600" />
            <div class="flex-1">
              <p class="text-sm font-bold text-red-900">💧 {{ waterHardBlock.message }}</p>
              <p class="text-xs mt-1 text-red-700">
      ...
      <div v-if="elecWarning || waterWarning" class="space-y-2 mb-4">
        <!-- Electricity Warning -->
        <div v-if="elecWarning" class="p-3 rounded-lg bg-amber-100 border-2 border-amber-500">
          <div class="flex items-start gap-2">
            <ExclamationTriangleIcon class="w-5 h-5 flex-shrink-0 mt-0.5 text-amber-600" />
            <div class="flex-1">
              <p class="text-sm font-bold text-amber-900">⚡ {{ elecWarning.message }}</p>
        ...
        <!-- Water Warning -->
        <div v-if="waterWarning" class="p-3 rounded-lg bg-amber-100 border-2 border-amber-500">
          <div class="flex items-start gap-2">
            <ExclamationTriangleIcon class="w-5 h-5 flex-shrink-0 mt-0.5 text-amber-600" />
            <div class="flex-1">
              <p class="text-sm font-bold text-amber-900">💧 {{ waterWarning.message }}</p>
              <p class="text-xs mt-1 text-amber-700">
                {{ waterWarning.detail }}
              </p>
            </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div
          :class="[
            ...
            activeInput === 'elec' ? 'bg-white ring-2 ring-blue-500 shadow-lg' : 'bg-white/70',
        ...
        <div
          :class="[
            'p-4 rounded-lg text-center cursor-pointer transition-all',
            activeInput === 'water' ? 'bg-white ring-2 ring-blue-500 shadow-lg' : 'bg-white/70',
