...
<template>
  <div class="p-6 md:px-8 bg-white rounded-xl shadow-md h-full flex flex-col">
      ...
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex-1 min-w-0">
          <div class="flex flex-col sm:flex-row sm:items-baseline sm:gap-x-4">
            ...
            <p class="mt-1 sm:mt-0 text-base text-slate-500">
              ภาพรวมและประวัติการแจ้งหนี้ทั้งหมด
            </p>
          </div>
        </div>
        <div class="mt-4 flex md:ml-4 md:mt-0">
          <BaseButton @click="openCreateRunModal" :icon="PlusIcon">
            สร้างรอบบิลใหม่
          </BaseButton>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto mt-6">
      ...
      >
        ...
        <p class="mt-1 text-sm text-slate-500">
          เริ่มต้นสร้างรอบบิลแรกของคุณได้เลย
        </p>
        <div class="mt-6">
          <BaseButton @click="openCreateRunModal" :icon="PlusIcon">
            สร้างรอบบิลแรก
    ...
    <CreateBillingRunModal
      ref="createRunModal"
      @success="handleCreateRunSuccess"
    />
  </div>
</template>
