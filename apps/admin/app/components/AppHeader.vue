...
const { status, data, signOut } = useAppAuth();
...
const mockUsersForSwitching = [
  { role: "OWNER", name: "เจ้าของหอ" },
  { role: "ADMIN", name: "ผู้จัดการ (Admin)" },
  { role: "STAFF", name: "พนักงาน (Staff)" },
];

const handleSwitchUser = async (role) => {
  console.log(`Switching to ${role}...`);
  // Logout ก่อน โดยไม่ redirect หน้า
  await signOut({ redirect: false });
  // จากนั้นค่อย redirect ไปหน้า login พร้อมส่ง role ที่ต้องการ
  await navigateTo(`/login?role=${role}`);
};
...
<template>
  ...
  >
    ...
    <div class="flex-1 flex items-center gap-x-4 self-stretch lg:gap-x-6">
      ...
      <div
        v-else
        class="flex items-center gap-x-3 text-sm font-semibold leading-6 text-gray-900"
      >
    ...
    <div class="flex items-center gap-x-4 lg:gap-x-6">
      <div class="flex items-center gap-2">
        ...
        >
          ...
          <div v-if="packageInfo.shimmer" class="shimmer-effect"></div>
        </div>
      </div>
