<template>
  <div class="space-y-3">
    ...
    >
      ...
      <p class="text-xs text-slate-400 mt-2">รองรับ: JPG, PNG, PDF (ไม่เกิน 5MB)</p>
    </div>

    <!-- Preview Area -->
    <div v-else class="relative">
      <div
        class="border-2 border-slate-200 rounded-xl overflow-hidden bg-white"
      >
        <img
          v-if="isImage"
          :src="previewUrl"
        ...
        <div
          v-else
          class="flex items-center justify-center p-8 bg-slate-50"
        >
...
const processFile = (file: File) => {
  ...
  if (!allowedTypes.includes(file.type)) {
    error.value = "ประเภทไฟล์ไม่ถูกต้อง กรุณาอัปโหลด JPG, PNG หรือ PDF";
    return;
  }
