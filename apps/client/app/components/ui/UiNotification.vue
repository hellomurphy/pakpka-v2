<template>
  <UPopover
    ...
    :ui="{
      content:
        'w-[90vw] max-w-sm p-0 overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5 bg-white',
    }"
  >
    <div class="relative inline-flex">
      ...
      />
      <span v-if="unreadCount > 0" class="absolute -top-0.5 -right-0.5 flex h-5 w-5">
        <span
    ...
    <template #content="{ close }">
      <div class="flex flex-col max-h-[60vh]">
        ...
        <div v-if="notifications.length > 0" class="overflow-y-auto">
          <ul class="divide-y divide-slate-100">
            <li v-for="item in notifications" :key="item.id">
              <NuxtLink
                ...
                :class="
                  item.isRead ? 'hover:bg-slate-50' : 'bg-indigo-50/60 hover:bg-indigo-100/70'
                "
              >
        ...
        <p v-else class="text-sm text-slate-500 text-center py-10">ยังไม่มีการแจ้งเตือน</p>

        <div class="border-t border-slate-100 px-4 py-3 bg-slate-50 text-center flex-shrink-0">
          <NuxtLink
            to="/notifications"
            @click="close()"
...
const getIconInfo = (type: string) => {
  switch (type) {
    case 'system':
      return { icon: 'i-lucide-settings', bgColor: 'bg-slate-200', iconColor: 'text-slate-600' }
    case 'dorm':
      return { icon: 'i-lucide-home', bgColor: 'bg-sky-100', iconColor: 'text-sky-600' }
    case 'announcement':
      return { icon: 'i-lucide-megaphone', bgColor: 'bg-amber-100', iconColor: 'text-amber-600' }
    case 'repair':
      return { icon: 'i-lucide-wrench', bgColor: 'bg-blue-100', iconColor: 'text-blue-600' }
    default:
      return { icon: 'i-lucide-bell', bgColor: 'bg-slate-200', iconColor: 'text-slate-600' }
  }
}
...
const getNotificationLink = (notification: {
  ...
  switch (notification.type) {
    ...
    case 'dorm':
      if (notification.subtype === 'parcel') return `/parcels/${notification.relatedId}`
      if (notification.subtype === 'bill') return `/payment/${notification.relatedId}`
      return '/'
    default:
      return '#'
  }
