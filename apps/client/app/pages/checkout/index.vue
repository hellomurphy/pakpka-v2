<template>
  <div class="max-w-2xl mx-auto bg-slate-50 pb-40">
    ...
    <div v-if="isFetchingInvoices" class="text-center pt-20 px-4">
      <Icon name="ph:spinner-duotone" class="text-4xl text-slate-400 animate-spin mb-4" />
    ...
    >
      <div class="max-w-md mx-auto">
        <div class="flex justify-between items-center mb-3">
          ...
          <span class="text-xl font-bold text-slate-800">{{ formatNumber(totalPrice) }} บาท</span>
        </div>
        <button
          ...
          @click="handlePayment"
        >
          <Icon v-if="isLoading" name="ph:spinner-duotone" class="h-5 w-5 animate-spin" />
...
const transformInvoice = (invoice: any) => {
  ...
  const lineItems = invoice.items.map((item: any) => {
    ...
    const relatedReading = invoice.meterReadings?.find((reading: any) => {
      ...
      return (
        (desc.includes('น้ำ') && readingType.includes('water')) ||
...
// คำนวณยอดรวม (บังคับเป็น number เผื่อ API ส่งเป็น string)
