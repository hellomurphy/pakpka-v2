<template>
  <div class="flex items-center justify-center bg-slate-50 p-4 text-center">
    <div v-if="paymentStatus === 'verifying'">
      <Icon name="ph:clock-countdown-duotone" class="text-6xl text-slate-400 animate-pulse" />
      <h1 class="text-xl font-bold mt-4 text-slate-700">กำลังรอการยืนยันการชำระเงิน</h1>
      <p class="text-slate-500 mt-1">กรุณาอย่าเพิ่งปิดหน้านี้ ระบบกำลังตรวจสอบกับธนาคาร</p>
    </div>
    ...
    <div v-else-if="paymentStatus === 'successful'">
      ...
      <p class="text-slate-600">เราได้รับยอดชำระของคุณเรียบร้อยแล้ว ขอบคุณที่ใช้บริการ</p>
      <button
        @click="goHome"
    ...
    <div v-else-if="paymentStatus === 'failed'">
      ...
      <h1 class="text-2xl font-bold mt-4 text-slate-800">การชำระเงินไม่สำเร็จ</h1>
      <p class="text-slate-600">เกิดข้อผิดพลาดบางอย่าง กรุณาลองใหม่อีกครั้งหรือเลือกช่องทางอื่น</p>
      <button @click="goBack" class="mt-8 bg-slate-600 text-white font-bold py-3 px-8 rounded-lg">
        ลองอีกครั้ง
      </button>
    </div>
  </div>
