<template>
  <div class="bg-slate-50">
    <main class="p-4 max-w-2xl mx-auto">
      <div
        class="flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm mb-6"
      >
        <img
      ...
      <div class="space-y-6">
        <section>
          <h3
            class="px-4 mb-2 text-sm font-semibold text-slate-500 uppercase tracking-wider"
          >
            บัญชี
          </h3>
            ...
            <ul class="divide-y divide-slate-100">
              <li v-for="item in accountMenu" :key="item.title">
                ...
                >
                  ...
                  <span class="font-semibold text-slate-700 flex-grow">{{
                    item.title
                  }}</span>
                  <Icon
        ...
        <section>
          <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            >
              ...
              />
              <span class="font-bold text-red-500 flex-grow text-left"
                >ออกจากระบบ</span
              >
            </button>
          </div>
...
const user = {
  ...
  pictureUrl:
    "https://profile.line-scdn.net/0h123abcdeFGHIJklmno_p1234567890t1234567890",
};
...
const myRoomLink = computed(() => {
  if (userRooms.value.length === 1) {
    return `/room/${userRooms.value[0].id}`; // ลิงก์ไปหน้ารายละเอียดเลย
  }
  return "/my-rooms"; // ลิงก์ไปหน้าสารบัญห้อง
});
